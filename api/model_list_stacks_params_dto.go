/*
Hautech API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hautechapi

import (
	"encoding/json"
)

// checks if the ListStacksParamsDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListStacksParamsDto{}

// ListStacksParamsDto struct for ListStacksParamsDto
type ListStacksParamsDto struct {
	OrderBy *string `json:"orderBy,omitempty"`
	Limit *float32 `json:"limit,omitempty"`
	Cursor *string `json:"cursor,omitempty"`
}

// NewListStacksParamsDto instantiates a new ListStacksParamsDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListStacksParamsDto() *ListStacksParamsDto {
	this := ListStacksParamsDto{}
	var orderBy string = "createdAt_DESC"
	this.OrderBy = &orderBy
	var limit float32 = 50
	this.Limit = &limit
	return &this
}

// NewListStacksParamsDtoWithDefaults instantiates a new ListStacksParamsDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListStacksParamsDtoWithDefaults() *ListStacksParamsDto {
	this := ListStacksParamsDto{}
	var orderBy string = "createdAt_DESC"
	this.OrderBy = &orderBy
	var limit float32 = 50
	this.Limit = &limit
	return &this
}

// GetOrderBy returns the OrderBy field value if set, zero value otherwise.
func (o *ListStacksParamsDto) GetOrderBy() string {
	if o == nil || IsNil(o.OrderBy) {
		var ret string
		return ret
	}
	return *o.OrderBy
}

// GetOrderByOk returns a tuple with the OrderBy field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListStacksParamsDto) GetOrderByOk() (*string, bool) {
	if o == nil || IsNil(o.OrderBy) {
		return nil, false
	}
	return o.OrderBy, true
}

// HasOrderBy returns a boolean if a field has been set.
func (o *ListStacksParamsDto) HasOrderBy() bool {
	if o != nil && !IsNil(o.OrderBy) {
		return true
	}

	return false
}

// SetOrderBy gets a reference to the given string and assigns it to the OrderBy field.
func (o *ListStacksParamsDto) SetOrderBy(v string) {
	o.OrderBy = &v
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *ListStacksParamsDto) GetLimit() float32 {
	if o == nil || IsNil(o.Limit) {
		var ret float32
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListStacksParamsDto) GetLimitOk() (*float32, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *ListStacksParamsDto) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given float32 and assigns it to the Limit field.
func (o *ListStacksParamsDto) SetLimit(v float32) {
	o.Limit = &v
}

// GetCursor returns the Cursor field value if set, zero value otherwise.
func (o *ListStacksParamsDto) GetCursor() string {
	if o == nil || IsNil(o.Cursor) {
		var ret string
		return ret
	}
	return *o.Cursor
}

// GetCursorOk returns a tuple with the Cursor field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListStacksParamsDto) GetCursorOk() (*string, bool) {
	if o == nil || IsNil(o.Cursor) {
		return nil, false
	}
	return o.Cursor, true
}

// HasCursor returns a boolean if a field has been set.
func (o *ListStacksParamsDto) HasCursor() bool {
	if o != nil && !IsNil(o.Cursor) {
		return true
	}

	return false
}

// SetCursor gets a reference to the given string and assigns it to the Cursor field.
func (o *ListStacksParamsDto) SetCursor(v string) {
	o.Cursor = &v
}

func (o ListStacksParamsDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListStacksParamsDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.OrderBy) {
		toSerialize["orderBy"] = o.OrderBy
	}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Cursor) {
		toSerialize["cursor"] = o.Cursor
	}
	return toSerialize, nil
}

type NullableListStacksParamsDto struct {
	value *ListStacksParamsDto
	isSet bool
}

func (v NullableListStacksParamsDto) Get() *ListStacksParamsDto {
	return v.value
}

func (v *NullableListStacksParamsDto) Set(val *ListStacksParamsDto) {
	v.value = val
	v.isSet = true
}

func (v NullableListStacksParamsDto) IsSet() bool {
	return v.isSet
}

func (v *NullableListStacksParamsDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListStacksParamsDto(val *ListStacksParamsDto) *NullableListStacksParamsDto {
	return &NullableListStacksParamsDto{value: val, isSet: true}
}

func (v NullableListStacksParamsDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListStacksParamsDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


